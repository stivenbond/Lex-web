...
using Lex.Module.FileProcessing.Core.Features.ProcessFile;
...
public sealed class FileProcessingController(IMediator mediator) : ControllerBase
{
    ...
    [HttpPost("process")]
    public async Task<IActionResult> ProcessFile([FromBody] ProcessFileCommand command, CancellationToken ct)
    {
        var result = await mediator.Send(command, ct);
        if (result.IsFailure) return BadRequest(result.Error);
